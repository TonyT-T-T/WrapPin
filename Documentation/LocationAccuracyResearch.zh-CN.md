...
调研分支新增了手动读取入口：先在 WrapPin 保持固定位置模拟，再进入“设置 → 连接状态 → 定位精度调研”，点“读取原始定位回调”。页面同时显示启动读取时的模拟目标、WrapPin 自己收到的最新 Core Location 坐标、水平精度、时间和软件模拟标记；停止读取或离开页面即清除，不写入诊断报告、持久化或遥测。若未收到新回调，不能把旧位置当作本次结果。这个读数来自 WrapPin 进程，仍需与 Apple 地图蓝点对照；它不能直接读取 Apple 地图进程内部的定位值。
