...
调研分支：`codex/location-accuracy-research`，从 `main` 的 `4ef99d1` 创建，现已合入 1.0.9 主分支 `681d119`。用户反馈的核心现象是 **WrapPin 中的位置与 Apple 地图的模拟定位蓝点约相差 1 公里**。Build 21 真机读数已证明 WrapPin 收到的 Core Location 数值与模拟目标完全相同；Apple 地图蓝点偏移的具体机制仍需验证。
...
- 目前仍无法读取 Apple 地图进程内使用的原始经纬度或定位来源。因此不能把这次偏移直接当作已证实的 MapKit 坐标系行为，更不能据此提交全局坐标补偿。
...
Build 22 增加了仅针对本次苏州坐标的可逆试验：当前固定模拟为 `31.316633, 120.677664` 时，“连接检测”页出现“苏州坐标系试验”。“试用候选模拟值”只更新原生会话的模拟坐标至 `31.318719, 120.673397`，保留 WrapPin 地图选点；“恢复原模拟坐标”可以切回原值。候选值由 GCJ-02→WGS84 近似反算所得，按同一算法回算到原地图坐标的误差小于 0.1 米；这个算法吻合不能替代 Apple 地图的真机验证。每次切换后必须重新读取原始回调，确认新数值确实进入 Core Location，再观察 Apple 地图蓝点。若回调已改为候选值且蓝点回到苏州中心，坐标系解释得到强支持；若回调已改变而蓝点仍偏，继续查 Apple 地图定位来源和呈现。试验中 WrapPin 地图选点仍在广场，而原生模拟目标暂时是另一组数值，这是刻意保留的对照。

Build 22 本地包为 `WrapPin-1.0.9-build22-suzhou-coordinate-trial.ipa`，SHA-256 为 `bee6afb1a0782cbfc44645533c25116b28a67266d226356ac11e11233836c7d4`。无签名 Release 归档和 ZIP 完整性检查通过，Build 核对为 22；尚未安装到真机，不能声称校正有效。它只提供受控试验按钮，不会对其他地点自动转换，也不是正式修复版。

本次最新回调的数字已经与模拟目标一致，排查焦点转为地图显示、坐标来源以及 Apple 地图是否采用该次模拟回调。只有后续受控对照证实具体转换边界，才能决定在哪个输入来源上转换；不应凭截图试错式加偏移。

静态检查与 Build 21 真机样本共同排除了这个固定点在 WrapPin 内部的数值精度损失，但不能证明 iOS 向所有 App 报告相同坐标，也不能单独证明中国大陆地图偏移的原因。当前最值得验证的是 GCJ-02 地图坐标与 WGS84 定位坐标的解释边界；不要在没有目标 App 或受控转换证据时加入统一偏移补偿。
