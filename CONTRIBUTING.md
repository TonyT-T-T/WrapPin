# Contributing to WrapPin

Thanks for helping improve WrapPin. Contributions should preserve its narrow purpose: location-based development and testing on an iPhone the user owns and controls.
...
The prebuilt `WrapPinPairingFFI.xcframework` allows normal app builds without compiling Rust. If the native bridge changes, follow `Documentation/BuildAndRelease.md` to rebuild both framework slices.
...
Include the WrapPin version/build, iOS version, connection type and reproducible steps. Redact addresses, coordinates, pairing information, Apple IDs, device names and other personal details. Prefer the app's readable diagnostics report over raw internal files.
