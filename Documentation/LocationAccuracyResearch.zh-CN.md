...
若暂时没有读取原始回调的工具，可做一次**仅用于诊断**的反向试验：先停止并恢复真实位置，再在 WrapPin 直接输入近似 WGS84 坐标 `31.318719, 120.673397`，启动固定模拟；观察 Apple 地图蓝点是否移到苏州中心广场，同时记录 WrapPin 自身标记在地图上的位置。这组测试值由上述非官方算法反推，只针对苏州这个样本，不应保存为正式地点或作为全局修复。
