...
struct UsageStatisticsPrivacyView: View {
    var body: some View {
        List {
            Section {
                Label {
                    Text("WrapPin sends only a small, fixed set of anonymous activity counts when sharing is enabled.")
                } icon: {
            ...
            Section("Storage and Control") {
                ...
                Text("Turning sharing off immediately stops new reporting and removes the identifier from WrapPin. It cannot withdraw anonymous events already received by TelemetryDeck.")
                    .foregroundStyle(.secondary)
                ...
                Text("WrapPin uses a narrow first-party sender instead of an analytics SDK, so extra device metadata cannot be added automatically.")
                    .foregroundStyle(.secondary)
            }
        }
