...
调研分支：`codex/location-accuracy-research`，基于 `main` 的 `4ef99d1`。用户反馈的核心现象是 **WrapPin 中的位置与 Apple 地图的模拟定位蓝点约相差 1 公里**。现已有苏州中心广场的截图和一组坐标，但仍缺 Apple 地图蓝点的原始定位回调，不能最终认定根因。
...
- 从截图和地点页仍无法确认 WrapPin 是搜索、手输坐标还是地图点选，也无法读取 Apple 地图蓝点的原始经纬度或定位来源。因此不能把这次偏移直接当作已证实的 MapKit 坐标系行为，更不能据此提交全局坐标补偿。
...
**第一轮继续测苏州中心广场这个固定地点**，使用同一台 iPhone、同一 iOS 版本。先记录此前是搜索结果、直接输入坐标还是地图点选；在 Apple 地图直接搜索 `31.316633, 120.677664`，查看搜索图钉是否落在蓝点附近。如果是，说明 Apple 地图对这组数字的解释与 WrapPin 选点地图不同，优先验证坐标系；如果不是，再查模拟服务及 Apple 地图定位来源。搜索图钉并不能代替原始 Core Location 回调的最终验证。
